...
export function useTimeline2Game() {
  ...
  const [lastPlacementCorrect, setLastPlacementCorrect] = useState<
    boolean | null
  >(null);
  const [isAnimating, setIsAnimating] = useState(false);
  const [correctlyPlacedCards, setCorrectlyPlacedCards] = useState<Set<number>>(
    new Set(),
  );
  ...
  const findCorrectPosition = useCallback(
    (card: Game, timeline: Game[]): number => {
