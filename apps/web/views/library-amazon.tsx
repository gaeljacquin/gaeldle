...
import { paginatedAmazonGamesQueryOptions } from '@/lib/services/game.service';
import { IconBrandAmazon } from '@tabler/icons-react';
import { cn } from '@workspace/ui/lib/utils';
import { DashboardHeader } from '@/components/dashboard-header';
import { useAmazonLibraryStore } from '@/lib/stores/game-list-store';
...
export function LibraryAmazonView() {
  const store = useAmazonLibraryStore();
  const { view, setView } = store;
  const filters = useGameListFilters({ store });

  const bulkDelete = useBulkDelete({
    queryKeysToInvalidate: ['amazon-games', 'games'],
    locationName: 'your Amazon library',
  });

  const { data, isLoading, isPlaceholderData, isFetching } = useQuery({
    ...paginatedAmazonGamesQueryOptions(
...
export default LibraryAmazonView;
