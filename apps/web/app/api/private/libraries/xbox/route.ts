...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!hasPagination && !q && !igdbId && !searchParams.has('sortBy')) {
      const data = await db
        .select(gameObject)
        .from(games)
        .where(eq(games.xbox, true))
        .orderBy(orderBy);
    ...
    console.error('Error fetching XBOX library:', error);

    return NextResponse.json(
      {
        success: false,
        error: 'Failed to fetch XBOX library',
      },
      { status: 500 },
    );
  }
