...
import {
  parseSortOption,
  type GameListFilterFormValues,
  type UseGameListFiltersReturn,
} from '@/lib/hooks/use-game-list-filters';
import type { SteamFilter } from '@/lib/services/game.service';

export interface UseSteamLibraryFiltersOptions {
  store: GameListStore;
}

const VALID_FILTERS: SteamFilter[] = ['all', 'owned', 'demos'];

export function useSteamLibraryFilters({
  store,
}: UseSteamLibraryFiltersOptions) {
  ...
  const urlFilter = searchParams.get('filter') as SteamFilter | null;
  ...
  // On mount, if the URL is missing parameters, sync the store values to the URL.
  useEffect(() => {
  ...
  return {
    form: form as UseGameListFiltersReturn['form'],
    formValues,
    filter,
    setFilter,
    debouncedSearch,
...
export type UseSteamLibraryFiltersReturn = ReturnType<
  typeof useSteamLibraryFilters
>;
